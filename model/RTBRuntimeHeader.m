...
+ (NSString *)headerForClass:(Class)aClass displayPropertiesDefaultValues:(BOOL)displayPropertiesDefaultValues {
    ...
    // top header
    [header appendString:[self topHeaderForClass:aClass]];
...
+ (NSString *)headerForProtocol:(RTBProtocol *)protocol {
    ...
    [header appendString:[self topHeaderForClass:Nil]];
...
+ (NSString *)topHeaderForClass:(Class)aClass {
    NSMutableString *header = [NSMutableString string];
    ...
    NSDictionary <NSString *, NSString *> *systemVersion = [NSDictionary dictionaryWithContentsOfFile:path];

    NSString *productName = systemVersion[@"ProductName"];
    if ([productName isEqualToString:@"iPhone OS"]) {
        productName = @"iOS";
    } else if ([productName isEqualToString:@"Mac OS X"]) {
        productName = @"macOS";
    }

    [header appendFormat:@"   Platform: %@ %@ (%@) %@\n",
            productName,
    ...
    if (aClass != Nil) {
        NSString *imagePath = @(class_getImageName(aClass));
        if (dyldRoot != nil && [imagePath hasPrefix:dyldRoot]) {
            imagePath = [imagePath substringFromIndex:dyldRoot.length];
        }
        [header appendFormat:@"   Image: %@", imagePath];

        NSString *parentFramework = imagePath;
        if ([parentFramework containsString:@".framework/"]) {
            while ([parentFramework lastPathComponent] != nil && ![[[parentFramework lastPathComponent] pathExtension] isEqualToString:@"framework"]) {
                parentFramework = [parentFramework stringByDeletingLastPathComponent];
            }
        }
        NSBundle *bundle = [NSBundle bundleWithPath:parentFramework];
        NSString *version = [bundle infoDictionary][@"CFBundleVersion"] ?: [bundle infoDictionary][@"CFBundleShortVersionString"];
        if (version != nil) {
